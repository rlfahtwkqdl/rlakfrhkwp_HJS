...
public class MoneyManager : MonoBehaviour
{
    ...
    // ★ MoneyManager.cs 내부에 이 함수를 추가해 주세요!
    public bool TrySpendMoney(int amount)
    {
        // 가진 누적 자산(totalMoney)이 소모할 금액보다 많다면 실행
        if (totalMoney >= amount)
        {
            totalMoney -= amount;
            SaveMoneyData(); // 차감 즉시 JSON에 저장 백업

            // UI 갱신을 위해 이벤트 실행 (세션 돈이 아닌 전체 돈 기반 UI를 위해 필요한 경우 대비)
            OnMoneyChanged?.Invoke(sessionMoney);

            Debug.Log($"[MoneyManager] {amount}원 소비 완료! 남은 총 자산: {totalMoney}원");
            return true; // 구매 성공
        }

        Debug.LogWarning("[MoneyManager] 잔액이 부족하여 구매할 수 없습니다.");
